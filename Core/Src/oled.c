...
static uint8_t oled_buffer[OLED_SCREEN_COLS][OLED_SCREEN_ROWS];

// 放到noinit段，不占用.bss，不会触发L6406内存溢出
// static uint8_t oled_buffer[OLED_SCREEN_COLS][OLED_SCREEN_ROWS] __attribute__((section(".noinit")));
...
int OLED_Init(OLED_TypeDef *OLED, OLED_InitTypeDef *OLED_InitStruct)
{
	...
	// 给缓冲区分配空间
	OLED->pBuffer = (uint8_t *)oled_buffer;
